...
export class AdminDashboardComponent implements OnInit {
  ...
  private async loadEntityCounts(): Promise<void> {
    try {
      ...
      // Все карточки, только названия и описания на русском
      this.entityCounts = [
        { name: 'Мероприятия', count: apiCounts.events, icon: 'calendar', color: '#3A72EC' },
        { name: 'Клубы', count: apiCounts.clubs, icon: 'users', color: '#10B981' },
        { name: 'Участники', count: apiCounts.participants, icon: 'user-check', color: '#F59E0B' },
        { name: 'Регистрации', count: apiCounts.registrations, icon: 'clipboard', color: '#8B5CF6' },
        { name: 'Спонсоры', count: apiCounts.sponsors, icon: 'star', color: '#EF4444' },
        { name: 'Площадки', count: apiCounts.venues, icon: 'map-pin', color: '#06B6D4' },
        { name: 'Волонтёры', count: apiCounts.volunteers, icon: 'heart', color: '#EC4899' },
        { name: 'Бюджеты', count: apiCounts.budgets, icon: 'dollar-sign', color: '#84CC16' },
        { name: 'Отделы', count: apiCounts.departments, icon: 'home', color: '#F97316' },
        { name: 'Результаты', count: apiCounts.results, icon: 'award', color: '#6366F1' },
        { name: 'Жюри', count: apiCounts.judges, icon: 'scale', color: '#14B8A6' },
        { name: 'Отзывы', count: apiCounts.feedback, icon: 'message-circle', color: '#F43F5E' },
      ];
    } catch (error) {
      console.error('Ошибка загрузки данных сущностей с API:', error);
  ...
  private async loadMockEntityCounts(): Promise<void> {
    // Все карточки, только названия и описания на русском
    const mockCounts = [
      { name: 'Мероприятия', count: 156, icon: 'calendar', color: '#3A72EC', change: '+12%', trend: 'up' as const },
      { name: 'Клубы', count: 24, icon: 'users', color: '#10B981', change: '+5%', trend: 'up' as const },
      { name: 'Участники', count: 2847, icon: 'user-check', color: '#F59E0B', change: '+18%', trend: 'up' as const },
      { name: 'Регистрации', count: 1923, icon: 'clipboard', color: '#8B5CF6', change: '+8%', trend: 'up' as const },
      { name: 'Спонсоры', count: 43, icon: 'star', color: '#EF4444', change: '+2%', trend: 'up' as const },
      { name: 'Площадки', count: 18, icon: 'map-pin', color: '#06B6D4', change: '0%', trend: 'neutral' as const },
      { name: 'Волонтёры', count: 287, icon: 'heart', color: '#EC4899', change: '+15%', trend: 'up' as const },
      { name: 'Бюджеты', count: 89, icon: 'dollar-sign', color: '#84CC16', change: '+7%', trend: 'up' as const },
      { name: 'Отделы', count: 12, icon: 'home', color: '#F97316', change: '0%', trend: 'neutral' as const },
      { name: 'Результаты', count: 134, icon: 'award', color: '#6366F1', change: '+22%', trend: 'up' as const },
      { name: 'Жюри', count: 56, icon: 'scale', color: '#14B8A6', change: '+4%', trend: 'up' as const },
      { name: 'Отзывы', count: 892, icon: 'message-circle', color: '#F43F5E', change: '+31%', trend: 'up' as const },
    ];
  ...
  private loadMockData(): void {
    // Fallback mock data
    this.entityCounts = [
      { name: 'Мероприятия', count: 156, icon: 'calendar', color: '#3A72EC' },
      { name: 'Клубы', count: 24, icon: 'users', color: '#10B981' },
      { name: 'Участники', count: 2847, icon: 'user-check', color: '#F59E0B' },
  ...
  getEntityDescription(entityName: string): string {
    const descriptions: { [key: string]: string } = {
      'Мероприятия': 'Активные мероприятия в этом семестре',
      'Клубы': 'Зарегистрированные студенческие клубы',
      'Участники': 'Всего зарегистрированных участников',
      'Регистрации': 'Регистрации на мероприятия за месяц',
      'Спонсоры': 'Активные спонсоры мероприятий',
      'Площадки': 'Доступные площадки для событий',
      'Волонтёры': 'Активные волонтёры',
      'Бюджеты': 'Утверждённые бюджеты мероприятий',
      'Отделы': 'Участвующие отделы',
      'Результаты': 'Опубликованные результаты',
      'Жюри': 'Доступные члены жюри',
      'Отзывы': 'Полученные отзывы участников'
    };
    return descriptions[entityName] || 'Информация о сущности';
  }
}
